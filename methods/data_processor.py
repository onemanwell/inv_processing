...
from methods.invoice_scrapper import extract_type, extract_nbr, extract_company_name, extract_concept, extract_currency, extract_dates, extract_importes
import pandas as pd
...
def logic_processor(documents: list[Document]) -> list[dict]:
    ...
    for doc in documents:
        ...
        totales = extract_importes(lines,concepto_data["y_reference"])

        total_general = totales.get("total_general")
        iva_minimo    = totales.get("iva_minimo_10")
        iva_basico    = totales.get("iva_basico_22")
        ...
        results.append({
            ...
            "data":{
                ...
                "Fecha_Documento":   extract_dates(lines).isoformat(),
